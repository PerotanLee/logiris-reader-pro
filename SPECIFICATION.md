...
### 2.4 知的なコンテンツ抽出 (フッター削除)
- **スマート・クリーンアップ**: Bloomberg特有の長大なフッター（購読解除リンク、推奨記事、広告等）を自動検知して削除。
- **コンテンツ保護 (Survival Tips)**: John Authers 氏のニュースレター等に含まれる「Survival Tips」セクションを誤削除から完全に保護する高度な文脈解析を実装。
- **More From Bloomberg Opinion**: 最新の仕様に基づき、記事推薦セクションもフッターの一部として適切にカット。
- **パフォーマンス**: 文字列ベースの高速スキャンにより、数ミリ秒で処理を完結。
...
### 1.3 リーダーモード (Pro版)
- **ハイブリッド抽出**: 外部サイトの有料記事を Logiris 内に取り込み、全文を快適な UI で表示。
- **Cookie Bridge**: ブラウザのログイン状態を GAS プロキシと安全に共有する仕組み。
- **再帰的ナビゲーション**: 記事内のさらなるリンクもリーダーモードで開き続ける機能。
